...
def duplicate_and_unique_movies(dataset, index_):
    ...
    print ("some examples of duplicated entry:")
    for i in range(0,len(dataset)):
        for j in range(i+1,len(dataset)):
            if(dataset[i][index_]==dataset[j][index_]):
                print("the row {} has duplicate entry in row {} at column {}".format(i,j,index_))
                dup_entry.append(dataset[i])
                dup_entry.append(dataset[j])
                max_review.append(max(int(dataset[i][12]),int(dataset[j][12])))
                count=count+1
    print("Number of duplicate entries are:")  
    print(count)       
...
def rate_bucket(dataset, rate_low, rate_high):
    rated_movies=[]
    for i in range(0,len(dataset)):
        if(((float(dataset[i][11])>=rate_low) and (float(dataset[i][11])<=rate_high))==True):
    ...
    Returns:
    rated_movies -- list of the details of the movies with required rating
    """
...
reviews_max={dup_entry[0][13]:max_review[0],dup_entry[2][13]:max_review[1],dup_entry[4][13]:max_review[2]}
...
def cleanmovies(seq):
    for i in range(0,len(seq)):
        if((int((seq[i][12]) not in max_review) == True) and (seq[i][13] in list(reviews_max.keys())==True)):
            del seq[i] 
    return seq       
movies_clean=cleanmovies(movies)
